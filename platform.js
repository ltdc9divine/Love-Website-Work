...
function buildPreviewUrl(template, customerData = {}) {
  const name1 = customerData.name1 || 'Minh';
  const name2 = customerData.name2 || 'Ngọc';
  const slug = slugify(`${name1}-${name2}`);
  const previewData = {
    slug,
    template: template.id,
    name1,
    name2,
    startDate: customerData.startDate || '2025-02-14',
    message: customerData.message || 'Một điều giản dị, nhưng rất đáng nhớ.',
    letter: customerData.letter || '',
    finalMessage: customerData.finalMessage || customerData.message || 'Yêu em hơn mỗi ngày.',
    photo1: customerData.photo1 || '',
    photo2: customerData.photo2 || '',
    album: Array.isArray(customerData.album) ? customerData.album.filter(Boolean) : [],
    loveLetter: customerData.letter || '',
    timeline: Array.isArray(customerData.timeline) ? customerData.timeline : []
  };
  ...
  return `${previewPath}?slug=${encodeURIComponent(slug)}&template=${encodeURIComponent(template.id)}`;
}
function openPreview(id) { selectedTemplate = templateRegistry.find((template) => template.id === id); $('#preview-category').textContent = selectedTemplate.categoryLabel; $('#preview-name').textContent = selectedTemplate.name; $('#preview-description').textContent = selectedTemplate.description; $('#preview-price').textContent = money(selectedTemplate.price); $('#preview-features').innerHTML = selectedTemplate.features.map((feature) => `<li>${feature}</li>`).join(''); $('#preview-iframe').src = buildPreviewUrl(selectedTemplate); $('#preview-dialog').showModal(); }
